use crate::rdiff::{constants::BLOCK_SIZE, io::RdiffFile};
...
fn test_rdiff_chunk_iterator_buffered_new_with_chunk_size_case1() {
    ...
    let iterator = BufferedRdiffChunkIterator::new_with_chunk_size(chunk_size, rdiff_file).unwrap();

    let rdiff_file = RdiffFile::new(file_name).unwrap();
    let buffer: Vec<u8> = Vec::new();
    let expected_iterator = BufferedRdiffChunkIterator {
        chunk_size,
        buffer,
        rdiff_file,
    };
